<header
    class="sticky top-0 flex w-full bg-white border-gray-200 z-99999 dark:border-border dark:bg-sidebar xl:border-b"
    x-data="{
    ...
    <div class="flex flex-col items-center justify-between grow xl:flex-row xl:px-6">
        <div
            class="flex items-center justify-between w-full gap-2 px-3 py-3 border-b border-gray-200 dark:border-border sm:gap-4 xl:justify-normal xl:border-b-0 xl:px-0 lg:py-4">

            <!-- Desktop Sidebar Toggle Button (visible on xl and up) -->
            <button
                class="hidden xl:flex items-center justify-center w-10 h-10 text-gray-500 border border-gray-200 rounded-lg dark:border-border dark:text-muted-foreground lg:h-11 lg:w-11"
                :class="{ 'bg-gray-100 dark:bg-card-hover': !$store.sidebar.isExpanded }"
            ...
            <button
                class="flex xl:hidden items-center justify-center w-10 h-10 text-gray-500 rounded-lg dark:text-muted-foreground lg:h-11 lg:w-11"
                :class="{ 'bg-gray-100 dark:bg-card-hover': $store.sidebar.isMobileOpen }"
            ...
            <x-header.theme-toggle class="flex xl:hidden ml-auto" />

            <!-- Application Menu Toggle (mobile only) -->
            <button @click="toggleApplicationMenu()"
                class="flex items-center justify-center w-10 h-10 text-gray-700 rounded-lg z-99999 hover:bg-gray-100 dark:text-muted-foreground dark:hover:bg-card-hover xl:hidden">
                <!-- Dots Icon -->
                ...
                <form action="{{ route('events.index') }}" method="GET" role="search">
                    <div class="relative">
                        <span class="absolute -translate-y-1/2 pointer-events-none left-4 top-1/2">
                            <!-- Search Icon -->
                            <svg class="fill-gray-500 dark:fill-muted-foreground" width="20" height="20"
                        ...
                        <input type="text" name="search" placeholder="Search events..."
                            class="h-11 w-full rounded-lg border border-gray-200 bg-transparent py-2.5 pl-12 pr-14 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-border dark:bg-input dark:text-foreground dark:placeholder:text-muted dark:focus:border-primary dark:focus:ring-primary/10 xl:w-[430px]" />
                        <button type="submit"
                            class="absolute right-2.5 top-1/2 inline-flex -translate-y-1/2 items-center gap-0.5 rounded-lg border border-gray-200 bg-gray-50 px-[7px] py-[4.5px] text-xs -tracking-[0.2px] text-gray-500 dark:border-border dark:bg-card-hover dark:text-muted-foreground">
                            <span> ⌘ </span>
                            <span> K </span>
                        </button>
                    </div>
        ...
        @if (request()->routeIs('dashboard') && auth()->check())
            ...
            <div class="flex w-full min-w-0 items-center px-5 py-3 xl:flex-1 xl:justify-center xl:px-4 xl:py-0">
                <div class="min-w-0">
                    <p class="truncate text-sm font-semibold text-gray-800 dark:text-foreground">{{ $headerGreeting }}, {{ $headerUser->name }}</p>
                    <p class="mt-0.5 hidden truncate text-xs text-gray-500 xl:block dark:text-muted-foreground">
                        @if ($headerMembership)
        ...
        <div :class="isApplicationMenuOpen ? 'flex' : 'hidden'"
            ...
            <div class="w-full xl:hidden">
                <form action="{{ route('events.index') }}" method="GET" role="search">
                    <div class="relative">
                        ...
                        <input type="text" name="search" placeholder="Search events..."
                            class="h-11 w-full rounded-lg border border-gray-200 bg-transparent py-2.5 pl-12 pr-14 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-border dark:bg-input dark:text-foreground dark:placeholder:text-muted dark:focus:border-primary dark:focus:ring-primary/10" />
                        <button type="submit"
                            class="absolute right-2.5 top-1/2 inline-flex -translate-y-1/2 items-center gap-0.5 rounded-lg border border-gray-200 bg-gray-50 px-[7px] py-[4.5px] text-xs -tracking-[0.2px] text-gray-500 dark:border-border dark:bg-card-hover dark:text-muted-foreground">
                            <span> ⌘ </span>
                            <span> K </span>
            ...
            <div class="flex items-center justify-between w-full gap-4 xl:w-auto xl:justify-end">
                <!-- Notification Bell + Theme Toggle (desktop only) -->
                <div class="flex items-center gap-2 2xsm:gap-3">
                    <!-- Notification Dropdown (Livewire - real-time) -->
                    @livewire('notification-bell')

                    <x-header.theme-toggle class="hidden xl:flex" />
                </div>
