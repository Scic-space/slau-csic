    ...
    <div class="space-y-6">
        <section class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-border dark:bg-white/[0.03] md:p-8">
            <div class="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
                ...
                <div class="rounded-lg border border-gray-200 bg-background p-5 dark:border-border dark:bg-background/60">
                    ...
                    @if (! empty($htbData))
                        <div class="mt-4 space-y-3 text-sm">
                            ...
                            <p class="rounded-md border border-gray-200 bg-white px-3 py-3 leading-6 text-gray-600 dark:border-border dark:bg-white/[0.03] dark:text-gray-400">
        ...
        <section class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-border dark:bg-white/[0.03]">
            ...
            <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
                ...
                <a href="{{ route('ctf.competition', $comp) }}" class="rounded-lg border border-gray-200 bg-background p-4 hover:border-emerald-300 dark:border-border dark:bg-background/60 dark:hover:border-emerald-700 transition">
                ...
                <div class="mt-8">
                    ...
                    <div class="mt-4 space-y-3">
                        @forelse ($leaderboard as $index => $entry)
                            <div class="flex items-center justify-between rounded-lg border border-gray-200 bg-background px-4 py-3 dark:border-border dark:bg-background/60">
            ...
            <div class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-border dark:bg-white/[0.03]">
                ...
                <div class="mt-5 space-y-4">
                    @foreach ($resources as $resource)
                        ...
                        <article class="rounded-lg border border-gray-200 bg-background p-4 dark:border-border dark:bg-background/60">
                            ...
                            <div class="mt-4 flex flex-wrap gap-3">
                                ...
                                @endif
                                <a href="{{ route('portal.competitions') }}" class="inline-flex items-center rounded-md border border-gray-300 px-3 py-2 text-sm font-medium text-gray-700 dark:border-border dark:text-gray-200">Related competitions</a>
                            </div>
                        </article>
                    @endforeach
                </div>
        ...
        <section class="space-y-4">
            ...
            @foreach ($resources as $resource)
                @php $progress = optional($resource->user_progress); @endphp
                <form method="POST" action="{{ route('portal.progress.update', $resource) }}" class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-border dark:bg-white/[0.03]">
                    @csrf
                    <div class="flex flex-wrap items-start justify-between gap-3">
                        <div>
                    ...
                    <div class="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-5">
                        <select name="status" class="rounded-md border border-gray-300 bg-white px-3 py-2.5 text-sm text-gray-800 dark:border-border dark:bg-card dark:text-white">
                        ...
                        <input type="number" name="progress_percentage" min="0" max="100" value="{{ $progress->progress_percentage ?? 0 }}" placeholder="Progress %" class="rounded-md border border-gray-300 bg-white px-3 py-2.5 text-sm text-gray-800 dark:border-border dark:bg-card dark:text-white">
                        <input type="number" name="completed_units" min="0" value="{{ $progress->completed_units ?? 0 }}" placeholder="Completed units" class="rounded-md border border-gray-300 bg-white px-3 py-2.5 text-sm text-gray-800 dark:border-border dark:bg-card dark:text-white">
                        <input type="number" name="score" min="0" value="{{ $progress->score ?? 0 }}" placeholder="Score" class="rounded-md border border-gray-300 bg-white px-3 py-2.5 text-sm text-gray-800 dark:border-border dark:bg-card dark:text-white">
                        <input type="text" name="ranking" value="{{ $progress->ranking }}" placeholder="Ranking" class="rounded-md border border-gray-300 bg-white px-3 py-2.5 text-sm text-gray-800 dark:border-border dark:bg-card dark:text-white">
                    </div>
                    <textarea name="notes" rows="3" placeholder="Notes on what you solved, what blocked you, or what you learned." class="mt-4 w-full rounded-md border border-gray-300 bg-white px-3 py-2.5 text-sm text-gray-800 dark:border-border dark:bg-card dark:text-white">{{ $progress->notes }}</textarea>
