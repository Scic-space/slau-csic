...
@section('content')
    <div class="space-y-3 py-4 sm:py-5">
        ...
        <section class="rounded-sm border border-gray-200 bg-white p-4 shadow-sm dark:border-border dark:bg-white/[0.03]">
            <p class="inline-flex items-center gap-2 text-sm font-semibold uppercase tracking-[0.22em] text-emerald-500"><span class="material-symbols-outlined" aria-hidden="true">school</span>Internal Online Classes</p>
        ...
        <section class="grid gap-3 xl:grid-cols-[0.95fr_1.05fr]">
            <div class="rounded-sm border border-gray-200 bg-white p-4 shadow-sm dark:border-border dark:bg-white/[0.03]">
                <h2 class="flex items-center gap-2 text-2xl font-semibold text-gray-900 dark:text-white"><span class="material-symbols-outlined text-emerald-500" aria-hidden="true">calendar_month</span>Scheduled class events</h2>
                    ...
                    @forelse ($classes as $class)
                        <article class="rounded-lg border border-gray-200 bg-background p-4 dark:border-border dark:bg-background/60">
                            <div class="flex flex-wrap items-start justify-between gap-3">
                                ...
                                <div class="rounded-md border border-gray-200 bg-white px-3 py-3 text-sm dark:border-border dark:bg-white/[0.03]">
                            ...
                            <div class="mt-4 flex flex-wrap gap-3 text-sm text-gray-500 dark:text-gray-400">
                                <span class="inline-flex items-center gap-1"><span class="material-symbols-outlined" aria-hidden="true">location_on</span>{{ $class->location }}</span>
                                <span class="inline-flex items-center gap-1"><span class="material-symbols-outlined" aria-hidden="true">how_to_reg</span>{{ $class->registered_count }} registered</span>
                                <span class="inline-flex items-center gap-1"><span class="material-symbols-outlined" aria-hidden="true">groups</span>{{ $class->attended_count }} attended</span>
                            </div>
                            <div class="mt-4 flex flex-wrap gap-3">
                                <a href="{{ route('events.show', $class->slug) }}" class="inline-flex items-center gap-2 rounded-sm border border-gray-300 px-3 py-2 text-sm font-medium text-gray-700 dark:border-border dark:text-gray-200"><span class="material-symbols-outlined" aria-hidden="true">visibility</span>View details</a>
                                @if ($class->external_link)
                                    <a href="{{ $class->external_link }}" target="_blank" rel="noreferrer" class="inline-flex items-center gap-2 rounded-sm bg-emerald-500 px-3 py-2 text-sm font-medium text-slate-950"><span class="material-symbols-outlined" aria-hidden="true">videocam</span>Join link</a>
                                @endif
                                @auth
            ...
            <div class="rounded-sm border border-gray-200 bg-white p-4 shadow-sm dark:border-border dark:bg-white/[0.03]">
                <h2 class="flex items-center gap-2 text-2xl font-semibold text-gray-900 dark:text-white"><span class="material-symbols-outlined text-emerald-500" aria-hidden="true">library_books</span>Class resource links</h2>
                <div class="mt-5 space-y-4">
                        ...
                        <article class="rounded-lg border border-gray-200 bg-background p-4 dark:border-border dark:bg-background/60">
                            ...
                            <div class="mt-4 flex flex-wrap gap-3">
                                ...
                                <form method="POST" action="{{ route('portal.progress.update', $resource) }}" class="flex flex-wrap items-center gap-3">
                                    ...
                                    <button type="submit" class="inline-flex items-center rounded-md border border-gray-300 px-3 py-2 text-sm font-medium text-gray-700 dark:border-border dark:text-gray-200">Keep tracked</button>
                                </form>
                            </div>
                        </article>
                    @endforeach
