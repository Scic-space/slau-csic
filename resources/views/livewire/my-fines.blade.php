<div class="py-6">
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <a href="{{ auth()->user()?->hasRole(['admin', 'super-admin']) ? route('filament.admin.pages.dashboard') : route('dashboard') }}" wire:navigate class="mb-5 inline-flex items-center gap-2 rounded-lg border border-violet-200 bg-violet-50 px-4 py-2 text-sm font-medium text-violet-700 hover:bg-violet-100 dark:border-violet-800 dark:bg-violet-900/20 dark:text-violet-400 dark:hover:bg-violet-900/30 transition-colors shadow-sm">
        ...
        <div class="mt-6 flex gap-3">
            <a href="{{ route('fines.export.csv') }}" class="rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700 focus:ring-2 focus:ring-gray-900 dark:focus:ring-white">
                Export CSV
            </a>
        </div>

        {{-- Pay Now Modal --}}
        @if ($showPaymentForm && $paymentFineId)
            <div class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/50" wire:click.self="closePayment">
                <div class="w-full max-w-lg rounded-xl bg-white dark:bg-gray-800 p-6 shadow-xl">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Submit Payment</h3>
        ...
        {{-- Appeal Modal --}}
        @if ($showAppealForm && $selectedFineId)
            <div class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/50" wire:click.self="closeAppeal">
                <div class="w-full max-w-lg rounded-xl bg-white dark:bg-gray-800 p-6 shadow-xl">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Submit Appeal</h3>
                    <form wire:submit="submitAppeal">
                        <div class="mb-4">
