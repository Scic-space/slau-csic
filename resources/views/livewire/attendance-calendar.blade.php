<div class="py-4 sm:py-5" x-data="attendanceCalendar(@js($records))"
    ...
    <div>
        ...
        <div class="mb-3 flex flex-wrap items-center gap-x-5 gap-y-2 rounded-sm border border-gray-200 bg-white px-4 py-3 text-sm shadow-sm dark:border-gray-700 dark:bg-gray-800">
        ...
        <div class="overflow-hidden rounded-sm border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
            <div class="flex flex-col gap-3 border-b border-gray-200 px-4 py-3 sm:flex-row sm:items-center sm:justify-between dark:border-gray-700">
                <div class="flex items-center gap-2">
                    <button type="button" @click="goToToday" class="rounded-sm border border-gray-200 px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-700">Today</button>
                    @foreach ([['prevMonth', 'chevron_left', 'Previous month'], ['nextMonth', 'chevron_right', 'Next month']] as [$action, $icon, $label])
                        <button type="button" @click="{{ $action }}" aria-label="{{ $label }}" class="inline-flex h-9 w-9 items-center justify-center rounded-sm border border-gray-200 text-gray-600 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700"><span class="material-symbols-outlined" aria-hidden="true">{{ $icon }}</span></button>
                    @endforeach
                </div>
            ...
            <div class="overflow-x-auto">
                <div class="min-w-[700px]">
                    <div class="grid grid-cols-7 border-b border-gray-200 bg-gray-50 text-center text-xs font-semibold uppercase tracking-wide text-gray-500 dark:border-gray-700 dark:bg-gray-900/50 dark:text-gray-400">
                    ...
                    <div class="grid grid-cols-7 gap-px bg-gray-200 dark:bg-gray-700">
                        <template x-for="day in calendarDays" :key="day.iso">
                            <button type="button" @dblclick="$wire.openCreateReminder(day.iso)" class="min-h-28 bg-white p-2 text-left hover:bg-gray-50 lg:min-h-32 dark:bg-gray-800 dark:hover:bg-gray-700/70" :class="{ 'bg-gray-50/80 dark:bg-gray-900/60': !day.isCurrentMonth }">
            ...
            <p class="border-t border-gray-200 px-4 py-2 text-xs text-gray-500 dark:border-gray-700 dark:text-gray-400">Tip: double-click a date to create a reminder for that day.</p>
        </div>
            ...
            <x-ui.drawer show="open" on-close="$wire.closeReminderDrawer()" title="{{ $editingReminderId ? 'Edit Event' : 'Add Event' }}">
                <form wire:submit="saveReminder" class="flex min-h-0 flex-1 flex-col">
                    <div class="flex-1 space-y-5 overflow-y-auto p-5">
                        ...
                        <div><label for="reminder-event-title" class="mb-1.5 block text-sm font-semibold text-gray-700 dark:text-gray-300">Event Title</label><input id="reminder-event-title" wire:model="reminderTitle" type="text" maxlength="120" placeholder="Enter event title" class="block w-full rounded-sm border border-gray-300 bg-white px-3 py-2.5 text-sm text-gray-900 focus:border-brand-500 focus:ring-brand-500 dark:border-gray-700 dark:bg-gray-800 dark:text-white">@error('reminderTitle')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror</div>
                        <fieldset><legend class="mb-2 text-sm font-semibold text-gray-700 dark:text-gray-300">Event Color</legend><div class="grid grid-cols-2 gap-3">
                            @foreach (['danger' => ['Danger', 'bg-red-500'], 'success' => ['Success', 'bg-green-500'], 'primary' => ['Primary', 'bg-brand-500'], 'warning' => ['Warning', 'bg-amber-500']] as $color => [$label, $swatch])
                                <label class="flex cursor-pointer items-center gap-2 rounded-sm border border-gray-200 px-3 py-2.5 text-sm text-gray-700 has-checked:border-brand-500 dark:border-gray-700 dark:text-gray-300"><input wire:model="reminderColor" type="radio" value="{{ $color }}" class="sr-only"><span class="h-3 w-3 rounded-full {{ $swatch }}"></span>{{ $label }}</label>
                            @endforeach
                        ...
                        @foreach ([['reminder-start', 'reminderStartsOn', 'Enter Start Date', 'calendar_today'], ['reminder-end', 'reminderEndsOn', 'Enter End Date', 'event']] as [$id, $model, $label, $icon])
                            <div>
                                ...
                                <div class="relative">
                                    <input id="{{ $id }}" x-ref="{{ str_replace('-', '', $id) }}" wire:model="{{ $model }}" type="date" @click="$el.showPicker?.()" class="block w-full cursor-pointer rounded-sm border border-gray-300 bg-white py-2.5 pl-3 pr-11 text-sm text-gray-900 focus:border-brand-500 focus:ring-brand-500 dark:border-gray-700 dark:bg-gray-800 dark:text-white">
                    ...
                    <div class="flex flex-col-reverse gap-3 border-t border-gray-200 p-5 sm:flex-row dark:border-gray-800">
