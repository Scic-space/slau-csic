@php
    $recentNotifications = auth()->check()
        ...
        : collect();
@endphp
<div class="relative" wire:poll.30s="refreshCount" x-data="notificationBell()" @click.away="closeDropdown()" @notification-updated.window="$wire.refreshCount()">
        ...
        <div class="flex items-center justify-between border-b border-gray-100 px-5 py-4 dark:border-gray-800">
            ...
            @if ($unreadCount > 0)
                <button wire:click="markAllRead" wire:loading.attr="disabled"
                        class="text-xs font-medium text-indigo-600 hover:text-indigo-500 dark:text-indigo-400 dark:hover:text-indigo-300">Mark all read</button>
            @endif
        </div>
